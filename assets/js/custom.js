...
$(document).ready(function () {
  ...
  // Reels Navigation
  function initializeReelsNavigation() {
    const $reelsScroll = $('.reels-scroll');
    ...
    $navDots.each(function (index) {
      $(this).on('click', function () {
        ...
        const scrollPosition = $reelItems.eq(index).position().left;
        $reelsScroll.animate({
    ...
    // Optional: Initial dot highlighting
    if ($navDots.length > 0) {
      $navDots.first().addClass('active');
    }
  }

  // Call reels navigation initialization
  initializeReelsNavigation();

  // Fallback vanilla JS reels navigation
  function vanillaReelsNavigation() {
    const reelsScroll = document.querySelector('.reels-scroll');
    const reelItems = document.querySelectorAll('.reel-item');
    const navDots = document.querySelectorAll('.nav-dot');

    if (reelsScroll && reelItems.length > 0 && navDots.length > 0) {
      navDots.forEach((dot, index) => {
        dot.addEventListener('click', () => {
          const scrollPosition = reelItems[index].offsetLeft;
          reelsScroll.scrollTo({
            left: scrollPosition,
            behavior: 'smooth'
          });
  ...
  // Call vanilla JS fallback
  vanillaReelsNavigation();

  // Debugging
  console.log('Reels items found:', $('.reel-item').length);
  console.log('Navigation dots found:', $('.nav-dot').length);
